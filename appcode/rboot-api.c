...
uint32 ICACHE_FLASH_ATTR rboot_get_slot_offset(uint8_t slot) {
    rboot_config conf;
    conf = rboot_get_config();
    if (slot >= conf.count) return (uint32)-1;
...
bool ICACHE_FLASH_ATTR rboot_verify_image(uint32 initial_offset, uint32 *image_length, const char **error_message)
{
    uint32 offset = initial_offset;
    char *error = NULL;
    DTXT("rboot_verify_image: verifying image at 0x%08x\n", initial_offset);
    if(offset % 4) {
    ...
    uint32 end_limit = offset + 0x100000;
    image_header_t image_header __attribute__((aligned(4)));
    if(spi_flash_read(offset, (uint32 *)&image_header, sizeof(image_header_t))) {
        error = "Flash fail";
        goto fail;
    }
    ...
    while(remaining_sections > 0 && offset < end_limit)
    {
        ...
        if(spi_flash_read(offset, (uint32 *)&header, sizeof(section_header_t))) {
            error = "Flash fail";
            goto fail;
        }

        DTXT("Found section @ 0x%08x (abs 0x%08x) length %lu load 0x%08x\n", offset-initial_offset, offset, (unsigned long)header.length, header.load_addr);
        ...
        if(!is_new_header) {
            ...
            for(int i = 0; i < header.length; i++) {
                if(i % sizeof(chunk) == 0)
                    spi_flash_read(offset+i, (uint32 *)chunk, sizeof(chunk));
        ...
        if(is_new_header) {
            ...
            spi_flash_read(offset, (uint32 *)&image_header, sizeof(image_header_t));
            offset += sizeof(image_header_t);
    ...
    spi_flash_read(offset-1, &read_checksum, 1);
    if((uint8_t)read_checksum != checksum) {
        error = "Invalid checksum";
        goto fail;
    }

    DTXT("rboot_verify_image: verified expected 0x%08x bytes.\n", offset - initial_offset);
...
bool ICACHE_FLASH_ATTR rboot_digest_image(uint32 offset, uint32 image_length, rboot_digest_update_fn update_fn, void *update_ctx)
{
    uint8_t buf[32] __attribute__((aligned(4)));
    for(int i = 0; i < image_length; i += sizeof(buf)) {
        if(spi_flash_read(offset+i, (uint32 *)buf, sizeof(buf)))
            return false;
        uint32 digest_len = sizeof(buf);
